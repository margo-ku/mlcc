...
public:
    ...
    void ResolveInstruction(const InstrPtr& instr,
                            InstrList& before,
                            InstrList& after);
    void FreeTemps();
...
private:
    ...
    void ResolveFloatImmediate(std::shared_ptr<ASMOperand>& operand,
                               InstrList& before);
    ...
    InstrList MakeLoadImmediateSequence(std::shared_ptr<Register> dst,
                                        uint64_t value,
                                        bool is_32bit);
