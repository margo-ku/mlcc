...
SemanticAnalyzer::SemanticAnalyzer(SymbolTable& symbol_table)
    : symbol_resolver_(symbol_table),
      declaration_type_checker_(symbol_table),
      expression_type_checker_(symbol_table),
...
void SemanticAnalyzer::Analyze(TranslationUnit* translation_unit) {
    RunPass(translation_unit, symbol_resolver_, symbol_resolver_.GetErrors()) &&
        RunPass(translation_unit, declaration_type_checker_,
                declaration_type_checker_.GetErrors()) &&
        RunPass(translation_unit, expression_type_checker_,
                expression_type_checker_.GetErrors()) &&
